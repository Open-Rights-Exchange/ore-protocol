...
ACTION oretoken::approve(name from, name to, asset quantity, string memo)
{
    ...
    check(from_account.balance.amount >= quantity.amount, "amount being approved is more than the balance of the approver account");
...
ACTION oretoken::transferfrom(name sender, name from, name to, asset quantity, string memo)
{
    ...
    check(allowance.amount >= quantity.amount, "the amount being transferred is more than the approved amount");
